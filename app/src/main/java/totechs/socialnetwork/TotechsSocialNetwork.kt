...
//
//            val newBlog = Blog(UUID.randomUUID().toString(), false, LocalDateTime.now(), LocalDateTime.now(), "Title", "Description", "ImageUrl",mutableListOf())
//            val newtag = Tag(UUID.randomUUID().toString(), false, LocalDateTime.now(), LocalDateTime.now(), newBlog.Id, "Name", "#FFFFFF")
//
//
//            blogRepository.AddAsync(newBlog, Dispatchers.IO)
//            blogRepository.AddTagToBlogByIdAsync(newBlog.Id, newtag, Dispatchers.IO)
        }
    }
}